...
def model_fn(features, labels, mode, params):
    ...
    _, (_, output_bw) = lstm_cell_bw(t, dtype=tf.float32,
                                     sequence_length=tf.reshape(nchars, [-1]))#we take last state 
    output = tf.concat([output_fw, output_bw], axis=-1)#concat on the last D dimension of tensors 50+50
    char_embeddings_lstm = tf.reshape(output, [-1, dim_words, params['char_lstm_size']*2])# [2,2,100]

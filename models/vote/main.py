...
def model_fn(features, labels, mode, params):
    ...
    char_embeddings_lstm = tf.reshape(output, [-1, dim_words, params['char_lstm_size']*2])# [b,tw,D]
    #char_embeddings_lstm = tf.reshape(output, [-1,dim_words ,dim_chars, params['char_lstm_size']*2])
    ...
    # Concatenate Word and Char Embeddings cnn
    embeddings2 = tf.concat([word_embeddings, char_embeddings_cnn], axis=-1)#concat on the last dimension axis 100+300
    embeddings2 = tf.layers.dropout(embeddings, rate=dropout, training=training)#50% de l'entrée
    ...
    output_cnn = tf.concat([output_fw2, output_bw2], axis=-1)
    output_cnn = tf.transpose(output_cnn, perm=[1, 0, 2])
    output_cnn = tf.layers.dropout(output_cnn, rate=dropout, training=training)	
	
	
    # CRF
    logits_lstm = tf.layers.dense(output, num_tags)#nn dense input : (output of bilstm), output dimension : same shape excpet last dim will be num of tags
    logits_cnn = tf.layers.dense(output_cnn, num_tags)
    crf_params = tf.get_variable("crf", [num_tags, num_tags], dtype=tf.float32)#variable of crf pars matrix num_tags*num_tags
    pred_ids, _ = tf.contrib.crf.crf_decode(logits_cnn, crf_params, nwords)#decode_tags: A [batch_size, max_seq_len] matrix, with dtype tf.int32. Contains the highest scoring tag indices.
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:#prediction
        ...
        log_likelihood_cnn, _ = tf.contrib.crf.crf_log_likelihood(
...
if __name__ == '__main__':
    ...
    params = {
        ...
        'lstm_size': 150,#word lstm unit number (hidden state size)
        'ATTENTION_SIZE': 50,
